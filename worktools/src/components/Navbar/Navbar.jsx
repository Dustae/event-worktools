  ...
  return (
    <>
      ...
      <nav className="w3-sidebar w3-bar-block w3-card w3-animate-left w3-hide-medium w3-hide-large" style={isToggled ? { display: "block", backgroundColor: "#7D9968" } : { display: "none", backgroundColor: "#7D9968" }} id="mySidebar">
        ...
        <NavLink to="/footer" className={({ isActive }) => (isActive ? 'w3-bar-item w3-button active' : 'w3-bar-item w3-button')} onClick={w3_close}>Footer</NavLink>
        <NavLink to="/promo" className={({ isActive }) => (isActive ? 'w3-bar-item w3-button active' : 'w3-bar-item w3-button')} onClick={w3_close}>Promo</NavLink>
        <NavLink to="/skill" className={({ isActive }) => (isActive ? 'w3-bar-item w3-button active' : 'w3-bar-item w3-button')} onClick={w3_close}>Skill</NavLink>
        <NavLink to="/statistics" className={({ isActive }) => (isActive ? 'w3-bar-item w3-button active' : 'w3-bar-item w3-button')} onClick={w3_close}>Statistics</NavLink>
      </nav>
    </>
  );
}
